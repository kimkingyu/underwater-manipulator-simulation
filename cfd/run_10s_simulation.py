...
import logging
logger = logging.getLogger('sim_10s')
logging.basicConfig(level=logging.INFO)

import ansys.fluent.core as pyfluent
from ansys.fluent.core.launcher.pyfluent_enums import UIMode

WORK_DIR = Path(r"D:\work4\cfd\wetted_arm_rebuild\articulated_mesh_v2\cad_attempt04\solve_10s_production")
...
try:
    ...
    print(f"Reading 415k case: {cas_file}...")
    ...
    # Outlet: prevent_reverse_flow=False with Direction Vector
    ...
    dt = 0.01
    total_time_s = 0.10 # initial verified segment
    ...
    sidecar = {
        "valid": True,
        "status": "running",
        "phase": "Phase A 10s Workflow Validation",
        "dt": dt,
    ...
    print(f"\n==================================================================")
    print(f"  Starting 10s cycle simulation: {total_steps} steps (dt={dt}s)...")
    print(f"==================================================================")
    ...
    for step in range(1, total_steps + 1):
        ...
        print(f"Step {step:2d}/{total_steps} (t={flow_time:5.2f}s, {elapsed:4.1f}s): "
    ...
    # Save checkpoint
    cp_case = WORK_DIR / "phaseA_step_0010.cas.h5"
    cp_data = WORK_DIR / "phaseA_step_0010.dat.h5"
    session.tui.file.write_case(str(cp_case))
    session.tui.file.write_data(str(cp_data))
    print(f"\nSaved checkpoint at {cp_case}")
    print("SUCCESS: 10s baseline simulation completed!")
finally:
